...
module Game
  ...
  class Ship
    ...
    def piu  
      ...
      if sound
        ...
        puts "Ooops! File does not exists!"
      end      
      piupiu = {}
      piupiu[:s] = Surface.new([5,5])
      piupiu[:s].fill(:red)
      piupiu[:y] = @rect.center[1]
      piupiu[:x] = @rect.center[0]
      @pius << piupiu
    end
    ...
    def update_piu( dt )
      return if @pius.empty?

      @pius.each do |piu|
        piu[:y] -= dt * @piu_speed
        if piu[:y] < 0 or piu[:y] > @screen.h
          @pius.delete(piu)          
        else
          piu[:s].blit(@screen,[ piu[:x], piu[:y] ])
        end        
      end
    end
  end
  ...
  class Runner
    ...
    def make_ship
      @ship = Ship.new( @screen, @screen.w/2, @screen.h/2 )
